...
namespace urlhandler.ViewModels
{
    public partial class MainWindowViewModel : ViewModelBase
    {
        ...
        public async Task Process()
        {
            ...
            {
                await MessageBoxManager.GetMessageBoxStandard("Error", "Another file is already being processed. Please wait for it to complete before running another.")
                    .ShowAsync();
            }
        }
            ...
            try
            {
                ...
                if (lastWriteTimeBeforeProcessing != lastWriteTimeAfterProcessing)
                {
                    ...
                    await Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
                    {
                        ...
                        Status = uploadSuccess ? "File uploaded successfully." : "Failed to upload file.";
                        await ShowNotificationAsync(Status);
            ...
            if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktopApp)
            {
                if (desktopApp.MainWindow!.WindowState == WindowState.Minimized)
                {
                    if (Environment.OSVersion.Platform == PlatformID.Win32NT &&
                        Environment.OSVersion.Version.Major >= 10 || Environment.OSVersion.Platform == PlatformID.Unix)
                    {
                        nf = new Notification
                        {
                            Title = title,
